...
func main() {
	...
	for r := range rCount {
		for c := range cCount {
			if grid[r][c] == "0" {
				endPositions := map[Coordinate]bool{}
				solve(grid, r, c, rCount, cCount, 0, endPositions)
				result += len(endPositions)
			}
		}
	}
	fmt.Println(result)
...
func solve(grid [][]string, r, c, rCount, cCount, num int, endPositions map[Coordinate]bool) {
	if grid[r][c] != fmt.Sprintf("%d", num) {
		return
	}
	if num == 9 {
		endPositions[Coordinate{r, c}] = true
		return
	}
